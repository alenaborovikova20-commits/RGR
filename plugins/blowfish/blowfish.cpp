...
static uint32_t F(uint32_t x, const uint32_t (*S)[256]) {
...
void blowfish_expand_key(const uint8_t* key, uint32_t* P, uint32_t (*S)[256]) {
    ...
    memcpy(S, S_INIT, sizeof(S_INIT));
    
    for (int i = 0; i < 18; i++) {
        uint32_t key_word = 0;
        for (int j = 0; j < 4; j++) {
            key_word = (key_word << 8) | key[(i * 4 + j) % BLOWFISH_KEY_BYTES];
        }
        P[i] ^= key_word;
    }
...
void blowfish_encrypt_block(const uint8_t* in, uint8_t* out, const uint32_t* P, const uint32_t (*S)[256]) {
    uint32_t L = ((uint32_t)in[0] << 24) | ((uint32_t)in[1] << 16) |
                 ((uint32_t)in[2] << 8) | (uint32_t)in[3];
    uint32_t R = ((uint32_t)in[4] << 24) | ((uint32_t)in[5] << 16) |
